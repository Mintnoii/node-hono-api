...
meta:
  layout: default
  menu: { show: true, order: 1, label: '关于', icon: database }
</route>
...
const column = computed(() => (false ? 1 : 2))
...
const latestBuildTime = new Date().toLocaleDateString()
</script>
...
<template>
  <NSpace vertical :size="16">
    <NCard title="关于" :bordered="false" size="small" segmented class="card-wrapper">
      <p>
        MintAdmin 是一个优雅且功能强大的后台管理模板，基于最新的前端技术栈，包括 Vue3, Vite5, TypeScript, Pinia 和
        UnoCSS。提供了一站式的后台管理解决方案，无需额外配置，开箱即用。它内置了丰富的主题配置和组件，代码规范严谨，实现了自动化的文件路由系统。
      </p>
    </NCard>
